 ...
 * @param {*} title 
 */
export function dingdingTitle(title = '资源超市') {
  dd.ready(function () {
  ...
  dd.ready(() => {
    ...
    if (isAndroid) {
      document.addEventListener('backbutton', (e) => { //安卓调用方式
        ...
        //    location.reload()
        // }
        if (window.location.hash === '#/order' || window.location.hash === '#/' || window.location.hash === '#/cart') {
          e.preventDefault() //阻止返回
      ...
      if (window.location.hash === '#/order' || window.location.hash === '#/' || window.location.hash === '#/cart') {
        ...
        dd.biz.navigation.setLeft({
          control: true,
          text: '',
          onSuccess: () => { //IOS调用方法 ios返回不起作用，需要手动让他返回window.history.go(-1)
            dd.biz.navigation.close({
              onSuccess: function () {
    ...
    dd.device.notification.confirm({
      ...
      onSuccess : function(result) {
        if(result.buttonIndex == 0) {
          location.href = BASELOGINURL+'/mcenterH5/login.html?corpId='+GetQueryString('corpId')
        }
      },
      onFail : function() {}
   });
